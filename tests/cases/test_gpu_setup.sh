...
sandbox="$(mktemp -d "${TMPDIR:-/tmp}/archportfolio-gpu-setup.XXXXXX")"
...
run() {
	PATH="$sandbox/bin:$PATH" HOME="$sandbox/h" XDG_CONFIG_HOME="$sandbox/h/.config" \
		XDG_STATE_HOME="$sandbox/h/.local/state" FACTS_OVERRIDE="$sandbox/none" PORTFOLIO_LANG=en \
...
backups="$sandbox/h/.local/state/archlinux-portfolio/backups"
