...
# The bundled wallpaper and the README captures under assets/screenshots are
# the only binaries this repository ships on purpose; every byte of them is
# still pinned by data/asset-manifest.tsv.
if find . \( -path ./.git -o -path ./.vm-test -o -path ./.vm-image -o -path ./dist -o -path ./.audit -o -path './tests/fixtures/*/sysroot/sys' \) -prune -o -type f ! -path './dotfiles/hypr/.local/share/wallpapers/warm-night.png' ! -path './assets/screenshots/*.png' -print0 | xargs -0 file | grep -Ev 'text|empty|SVG|JSON|Python script|shell script' >/dev/null; then
